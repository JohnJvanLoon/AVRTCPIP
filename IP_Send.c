...
uint8_t IP_Send_Start (void)
{
	...
	{
		IP_send_comm_data.state=S1;  //move to state 1 and return 1
		ret_val=1;
	}
	return ret_val;
}
...
uint8_t IP_Send_Release (void)
{
	...
	if (ETH_Send_Release())
	{
		if (IP_send_comm_data.state!=complete) //if if IP send is not equal to complete do nothing
		{
		}
			IP_send_comm_data.state=idle; //otherwise goes to idle and return 1
			ret_val=1;
	}
